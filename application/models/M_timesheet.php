...
Class M_timesheet extends CI_Model{
    ...
    function Timesheet_bydate($user_id,$date){
        ...
        //echo "date : $date<br>";
        $query = $this->db->query("
          SELECT HOUR_TOTAL FROM TIMESHEET 
JOIN WBS_POOL ON  WBS_POOL.WP_ID=TIMESHEET.WP_ID
JOIN RESOURCE_POOL ON  RESOURCE_POOL.RP_ID=WBS_POOL.RP_ID
where TS_DATE=to_date('$date','yyyy-mm-dd') AND RESOURCE_POOL.USER_ID = '".$user_id."'");
        $hasil = $query->result_array();
        //echo $this->db->last_query();
        //die;
        return $hasil["HOUR_TOTAL"];
    ...
    function editTimesheetTemp($data,$rh_id){
        ...
        //1 = have a new data
            ...
            $this->db->query("UPDATE TEMPORARY_TIMESHEET 
                             set IS_APPROVED = '-1',TS_ID = '$TS_ID' , SUBJECT='$SUBJECT', MESSAGE = '$MESSAGE', HOUR_TOTAL = '$HOUR_TOTAL', TS_DATE ='$TS_DATE', WP_ID='$WP_ID', LATITUDE='$LATITUDE', LONGITUDE='$LONGITUDE',IS_VALID=1,SUBMIT_DATE = to_timestamp('$SUBMITDATE','YYYY-MM-DD HH24:MI:SS'),ACTION = 'edit',RH_ID='$rh_id' 
                           where TS_ID = '$TS_ID'
                             ");
