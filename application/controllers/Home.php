...
//$iwo=$this->M_detail_project->getProjectIWO($id);
        */
        $data['project_detail']=$this->db->query("select iwo_no as iwo, category as effort_type, project_type, project_name,bu_name as bu_owner, project_desc as description, project_status,pm_id,user_id 
        from projects join p_bu on projects.bu_code = p_bu.bu_code 
        join p_project_category ppc on projects.type_of_effort=ppc.id
        join users pm on pm.user_id=projects.pm_id
...
///
    ...
    public function projectdoc(){
        ...
        $this->datajson['project_doc_list'] = $this->db->query("select a.*,b.user_name from (
                                                                  select project_id,doc_name,url,date_upload,upload_by,doc_desc,'document' as jenis from project_doc
                                                                  UNION 
                                                                  select project_id,evidence as doc_name,evidence as url,submit_date as date_upload ,request_by as upload_by,reason as doc_desc,'rebaseline' as jenis from rebaseline_history
                                                                ) a
                                                                join users b
